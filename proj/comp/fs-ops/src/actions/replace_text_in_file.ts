...
export async function handle__replace_text_in_file(action: SlupeAction): Promise<FileOpResult> {
  ...
  try {
    ...
    if (count > 1) {
      return {
        success: false,
        error: `replace_text_in_file: old_text appears ${count} times, must appear exactly once`
      };
    }
